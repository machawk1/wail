...
const AddSeedLocation = ({match, nukeCheckUrl}) => (
  <span style={{margin: 0, padding: 0}}><Link onClick={nukeCheckUrl} style={linkStyle} to={routeNames.selectCol}>Collections</Link> > <Link
    style={linkStyle}
    onClick={nukeCheckUrl}
    to={drr.viewCollection(match.params.col)}>{match.params.col}</Link> > Add Seed</span>
)
